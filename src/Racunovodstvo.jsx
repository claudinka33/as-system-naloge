...
export const RACUNOVODSTVO_KATEGORIJE = {
  ...
  kompenzacije: {
    ...
    subKategorije: [
      'Medsebojna', 'Verižna', 'E-kompenzacija (AJPES)', 'Drugo'
    ],
  },
  predcasna_placila: {
...
// GLAVNA KOMPONENTA
// =====================================
export default function Racunovodstvo({ currentUser, isAdmin }) {
  const [entries, setEntries] = useState([]);
  const [loading, setLoading] = useState(true);
  const [selectedCategory, setSelectedCategory] = useState(null); // null = pregled vseh
  ...
  async function loadEntries() {
    ...
    try {
      ...
      // Če tabela ne obstaja, pokaži prazno stanje (uporabnik bo dobil SQL skript)
      setEntries([]);
    } finally {
      setLoading(false);
    }
  ...
  // Število vnosov po kategoriji (za prikaz na kartici)
  const countByCategory = (catKey) => entries.filter(e => e.category === catKey).length;

  // Pravice: vsi admini lahko gledajo, urejajo pa Sara + drugi admini
    ...
    return (
      <div className="bg-white border border-as-gray-200 rounded-xl p-12 text-center">
        ...
        <p className="text-as-gray-400 text-sm mt-1">
          Za dostop kontaktiraj Saro ali Aleša.
        </p>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="bg-white border border-as-gray-200 rounded-xl p-4 mb-4 shadow-sm">
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <button
            ...
            className="px-4 py-2 text-white rounded-lg flex items-center gap-2 font-semibold hover:shadow-md transition"
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
        <button
          ...
          className={`bg-white border rounded-xl p-3 text-left transition hover:shadow-md ${statusFilter === 'all' ? 'border-as-red-400 ring-2 ring-as-red-100' : 'border-as-gray-200'}`}
        >
        ...
        {STATUSI.map(s => (
          <button
            ...
            className={`bg-white border rounded-xl p-3 text-left transition hover:shadow-md ${statusFilter === s.key ? 'ring-2 ring-as-red-100 border-as-red-400' : 'border-as-gray-200'}`}
          >
      ...
      <div className="bg-white border border-as-gray-200 rounded-xl p-3 mb-4 shadow-sm flex items-center gap-2 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            type="text"
            placeholder="Išči po naslovu, opisu, partnerju ..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-9 pr-3 py-2 border border-as-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-as-red-100 focus:border-as-red-300"
          />
        </div>
        {selectedCategory && (
          <button
            onClick={() => setSelectedCategory(null)}
            className="px-3 py-2 bg-as-gray-100 hover:bg-as-gray-200 rounded-lg text-sm font-semibold flex items-center gap-1"
          >
            <X className="w-4 h-4" />
            Vse kategorije
          </button>
      ...
      {Object.entries(RACUNOVODSTVO_KATEGORIJE).map(([key, cat]) => {
        ...
        return (
          <button
            ...
            className="bg-white border border-as-gray-200 rounded-xl p-4 text-left hover:shadow-lg hover:border-as-gray-300 transition group flex items-start gap-3"
          >
            <div 
  ...
  if (entries.length === 0) {
    return (
      <div className="bg-white border border-as-gray-200 rounded-xl p-12 text-center">
        ...
        <p className="text-as-gray-400 text-sm mt-1">
          Klikni "Nov vnos" za dodajanje prvega vnosa.
        </p>
      </div>
    );
  }
...
function EntryRow({ entry, onEdit, onDelete, canEdit }) {
  ...
  return (
    <div className="bg-white border border-as-gray-200 rounded-xl shadow-sm overflow-hidden">
      <div 
        className="p-3 hover:bg-as-gray-50 cursor-pointer transition"
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              {entry.sub_category && (
                <span className="text-xs text-as-gray-500 bg-as-gray-100 px-2 py-0.5 rounded">
                  {entry.sub_category}
                </span>
              )}
            </div>
            ...
            <div className="flex items-center gap-3 flex-wrap text-xs text-as-gray-500">
              ...
              {entry.amount && (
                <span className="font-semibold text-as-gray-700">
              ...
              {entry.due_date && (
                <span className="flex items-center gap-1">
                  ...
                  Rok: {formatDate(entry.due_date)}
                </span>
              )}
              {entry.payment_date && (
                <span className="flex items-center gap-1 text-emerald-600">
                  <CheckCircle2 className="w-3 h-3" />
                  Plačano: {formatDate(entry.payment_date)}
                </span>
              )}
            </div>
          </div>
          ...
          <ChevronDown 
            className={`w-4 h-4 text-as-gray-400 flex-shrink-0 mt-1 transition ${expanded ? 'rotate-180' : ''}`}
          />
        </div>
      </div>
      ...
      {expanded && (
        <div className="border-t border-as-gray-100 bg-as-gray-50/50 p-3 space-y-2">
          ...
          <div className="text-xs text-as-gray-400 pt-1">
            Ustvarjeno: {entry.created_by_name} · {formatDate(entry.created_at)}
          </div>
          ...
          {canEdit && (
            <div className="flex items-center gap-2 pt-2 border-t border-as-gray-200">
              <button
                ...
                className="px-3 py-1.5 bg-white border border-as-gray-200 rounded-lg text-xs font-semibold flex items-center gap-1 hover:bg-as-gray-50"
              >
                <Edit2 className="w-3 h-3" /> Uredi
              </button>
              <button
                onClick={(e) => { e.stopPropagation(); onDelete(); }}
                className="px-3 py-1.5 bg-white border border-red-200 text-red-600 rounded-lg text-xs font-semibold flex items-center gap-1 hover:bg-red-50"
              >
      ...
      <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full max-w-2xl max-h-[95vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-as-gray-200 px-5 py-3 flex items-center justify-between">
          <h2 className="text-lg font-bold text-as-gray-700">
            {entry?.id ? 'Uredi vnos' : 'Nov vnos'}
          </h2>
          <button
            onClick={onClose}
            className="p-1.5 hover:bg-as-gray-100 rounded-lg text-as-gray-400"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 border border-as-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-as-red-100 focus:border-as-red-300"
            >
          ...
          <div>
            ...
            <input
              ...
              placeholder="npr. Račun Elektro Maribor 11/2025"
              required
              className="w-full px-3 py-2 border border-as-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-as-red-100 focus:border-as-red-300"
            />
          </div>
          ...
          <div>
            <label className="block text-xs font-bold text-as-gray-500 uppercase tracking-wider mb-1">
              Partner / dobavitelj / kupec
            </label>
            <input
              type="text"
              value={form.counterparty}
              onChange={(e) => setForm({...form, counterparty: e.target.value})}
              placeholder="npr. Elektro Maribor d.d."
              className="w-full px-3 py-2 border border-as-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-as-red-100 focus:border-as-red-300"
            />
          </div>
          ...
          <div className="flex items-center gap-2 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 bg-as-gray-100 hover:bg-as-gray-200 text-as-gray-700 rounded-lg font-semibold text-sm"
            >
              Prekliči
            </button>
            <button
              type="submit"
              disabled={saving}
              className="px-4 py-2 text-white rounded-lg font-semibold text-sm flex items-center gap-2 disabled:opacity-50"
              style={{backgroundColor: '#C8102E'}}
            >
